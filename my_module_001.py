...
COMMAND_LIST = ["search", "update", "help", "exit", "printall",
                "export", "goto"]  # rear 2 are search only command
...
def help_menu(cmd=""):
    if cmd == "":
        print(
            "To find out detailed information of certain command, please enter \"help [COMMAND_NAME]\"")
        ...
        print("\tinfo            Print information of the data loaded.\n")
        ...
        print(
            "usage: search [search_in] [-title] [-tag] [-excerpt] [-sort] \n")
...
def info(d: dict):
    print("latest update: " + d["update"])
    print(d["count"], "article(s): technews:", len(
        d["technews"]), ", cool3c:", len(d["cool3c"]), end="\n\n")
...
def nrm_search(search_in: dict, search: str, technews=True, cool3c=True, sbt=False) -> list:
    ...
    [result.append(elm) for elm in searchByTag(search_in, search,
                                               technews=technews, cool3c=cool3c, display=False) if elm not in result]
    [result.append(elm) for elm in searchByTitleKeyword(search_in, search,
                                                        technews=technews, cool3c=cool3c, display=False) if elm not in result]
    [result.append(elm) for elm in searchInArticleExcerpt(
        search_in, search, technews=technews, cool3c=cool3c, display=False) if elm not in result]
    printSearchResultMenu(result, sbt=sbt)
    return result
...
def init(file="waterball.json") -> dict:
    ...
    try:
        ...
        print(d["count"], "article(s): technews:", len(
            d["technews"]), ", cool3c:", len(d["cool3c"]))

    except:
        print("ERROR: file \""+file +
              "\" not found or invalid, try update() before accessing the data.")
    return d
...
def printSearchResultMenu(toPrint: list, sbt=False):
    ...
    for elm in toPrint:
        ...
        if i % 10 == 0:
            while True:
                opt = input(
                    "Enter command or press enter to go to next page> ")
                if opt == "":
                    print("\r", end="")
                    break
                else:
...
def update(technews=True, cool3c=True) -> dict:
    ...
    if technews:
        ...
        for j in range(2, 100):  # otherpages
            req = requests.get(
                "https://technews.tw/author/handymantw/page/"+str(j)+"/")
            soup = bs4.BeautifulSoup(req.text, "html.parser")
            title = soup.select("h1.entry-title a")
            content = soup.select("div.moreinf")
            time = soup.select("span.body")[1::4]
    ...
    if cool3c:
        ...
        for i in range(len(title)):
            ...
            if title[i]["href"] in untagged:
                ...
                articles["cool3c"][-1]["tags"] = list(
                    set(tags[i].text.replace("\n\n", "").split("\n")))
                #alltag.update(list(set(tags[i].text.replace("\n\n", "").split("\n"))))
            content_count += 1

        for j in range(1, 100):  # other pages
            req = requests.get(
                "https://www.cool3c.com/author/tiramisu/p"+str(j))
            soup = bs4.BeautifulSoup(req.text, "html.parser")
            title = soup.select("div.title a")
            content = soup.select("div.content a")
...
def calTime(timeformat: str, cool3c_link="") -> str:
    ...
    if cool3c_link != "":  # n天前 n月前 n年前
        ...
        articleTime = date.datetime(
            int(formatted[0]), int(formatted[1]), int(formatted[2]))
        return str(articleTime).split()[0]
    else:  # technews
        formatted = timeformat.replace(" ", "").replace(
